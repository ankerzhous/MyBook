...
​					@**Import**(AutoConfigurationPackages.Registrar.class)，Spring的底层注解@Import，

​						给容器中导入组件AutoConfigurationPackages.Registrar.class

**重要含义**：将主配置类（@SpringBootApplication标注的类）的所在包及下面所有子包里面的所有组件扫描到Spring容器；**所以通常讲标注了@SpringBootApplication的类放在com.zsy下方便扫描其他包**
...
1、配置类**@Configuration** 表明该类是个配置类，

2、使用**@Bean**给容器中添加组件，id是方法名
